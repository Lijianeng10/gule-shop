    ...
    <div class="base-scroll"
        ...
        <section class="inner" :style="{top:(top-offsetPullRefreshHeight)+'px'}">
            ...
            <footer class="align-center load-more" v-show="!scrollFooterHidden">
                <slot name="load-more">
                    <span :class="{'no-more':noMoreData}">{{noMoreData?'沒有更多数据':'加载中......'}}</span>
                </slot>
            </footer>
        </section>
    </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                disFunctionScroll: false, //禁止由方法出发的自动滚动(聊天的时候，用户操作禁止自动触底)
                scrollEndTimer: false
            }
        },
        mounted() {
        ...
        methods: {
            touchStart(e) {
                this.touchScrollTop = this.$el.scrollTop;
                this.disFunctionScroll = true;
            ...
            touchEnd() {
                ...
                // 延时反馈，以防在部分手机中，touch速度过快时scrollTop不能及时更新，从而导致检测scrollTop都是无变化的
                setTimeout(() => {
                    if(this.touchScrollTop === this.$el.scrollTop) {
                        this.touchScroll = false;
                        this.$forceUpdate();
                    }
                }, 100);
            ...
            onScroll() {
                ...
                this.scrollEndTimer = setTimeout(() => {
                    if(bottom > this.$el.querySelector('.load-more').clientHeight || this.noMoreData) {
                        this.touchScroll = false;
                        this.$forceUpdate();
                    }
                }, 100);
                if(bottom < this.$el.querySelector('.load-more').clientHeight && !this.noMoreData) this.infinite();
                this.$emit('scrolling', this.$el);
            },
            setScrollTop() {
                if(this.$store.state.config.includedPage.includes(this.$parent.$options.name) || this.$store.state.controls.scroll.positionInclude.includes(this.$route.path)) {
                    this.$store.commit("SET_CONTROLS_SCROLL_POSITION", {
            ...
            toScrollTop(top) {
                if(this.disFunctionScroll) return;
